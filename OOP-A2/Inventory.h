...
public:
    ...
    MusicFestival get_item(size_t i) { return (i < _count) ? _items[i] : MusicFestival{}; }
    ...
    void add_item(int bandnumber, string bandname, string city, double price);

    // Looks for a matching abstraction object and returns the first found or default object
    MusicFestival find_item(MusicFestival query);
